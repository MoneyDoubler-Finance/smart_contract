...
function main() {
  ...
  // 3) Read committed IDL: prefer staged index, then HEAD, then working tree
  function tryGitShow(ref) {
    const r = run('git', ['show', ref], { cwd: repoRoot });
    return r.status === 0 ? r.stdout : null;
  }
  let committedContent = tryGitShow(':target/idl/pump.json');
  if (committedContent == null) {
    committedContent = tryGitShow('HEAD:target/idl/pump.json');
  }
  if (committedContent == null) {
    try {
      committedContent = readFileSync(idlPath, 'utf8');
    } catch (e) {
      console.error('[idl-check] Committed IDL target/idl/pump.json not found (index/HEAD/workspace).');
      process.exit(3);
    }
  }
