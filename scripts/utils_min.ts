...
export function loadProgramFromIdl(path: string = 'target/idl/pump.json', provider?: anchor.AnchorProvider): MinimalProgramBundle {
  ...
  const program = new anchor.Program(idlClean as anchor.Idl, programId, resolvedProvider);
  return { idl: idlClean as anchor.Idl, programId, program, provider: resolvedProvider };
}

export function deriveGlobalConfigPda(programId: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync([Buffer.from('global-config')], programId);
  return pda;
}

export function deriveBondingCurvePda(programId: PublicKey, mint: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync([Buffer.from('bonding-curve'), mint.toBuffer()], programId);
  return pda;
}

export function getCurveTokenAccount(mint: PublicKey, bondingCurve: PublicKey): PublicKey {
  return getAssociatedTokenAddressSync(mint, bondingCurve, true, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
}

export function getOwnerTokenAccount(mint: PublicKey, owner: PublicKey): PublicKey {
